 /*
 * File Upload, based on CKAN image-upload.js
 */
this.ckan.module('ogdch_file_upload', function($) {
  return {
    /* options object can be extended using data-module-* attributes */
    options: {
      is_url: false,
      is_upload: false,
      field_upload: 'file_upload',
      field_url: 'file_url',
    ...
    initialize: function () {
      ...
      this.field_file = this.input.parents('.form-group');
      this.field_url_input = $('input', this.field_url);
      this.field_name = this.el.parents('form').find(field_name);
      // this is the location for the upload/link data/file label
      this.label_location = $('label[for="field-file-url"]');
      ...
      if (this.previousUpload) {
        $('<div class="error-inline"><i class="fa fa-warning"></i> ' +
          this._('Please select the file to upload again') + '</div>').appendTo(this.field_file);
      }
    ...
    _fileNameFromUpload: function(url) {
      // If it's a local CKAN file return the entire URL.
      if (/^\/base\/files/.test(url)) {
        return url;
      }
