...
def get_showcase_type_name(showcase_type, lang_code):
    ...
    return ogdch_localize_utils.get_localized_value_from_json(type_string, lang_code)


def localize_showcase_facet_title(facet_item):
    return get_showcase_type_name(facet_item["display_name"], lang_code=lang())
...
def ogdch_resource_display_name(res):
    ...
    if not resource_display_name:
        try:
            pkg = tk.get_action("package_show")({}, {"id": res["package_id"]})
            resource_display_name = get_localized_value_for_display(pkg.get("title"))
            if not resource_display_name:
                return pkg["name"]
        except (
            tk.ObjectNotFound,
...
def ogdch_get_switch_connectome_url(identifier):
    """Construct a url to the SWITCH connectome site using the package
    identifier.
    """
    return tk.config.get("ckanext.switzerland.switch_connectome_base_url", "") + quote(
